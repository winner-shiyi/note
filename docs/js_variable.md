...
二、变量的命名必须以字母，或者下划线（\_），或者美元符（$）开头，余下的部分可以是任意的字母，数字，或者是（\_） 或者是（$）组合而成，不允许数字开头

三、不能用关键字或者是保留字命名

ECMAScript 关键字列表：（http://www.w3school.com.cn/js/pro_js_keywords.asp）

ECMAScript 保留字列表：（http://www.w3school.com.cn/js/pro_js_reservedwords.asp）
...
## 变量声明方法：
    ...
    1.变量如何创建(声明)
     ...
     3.不用var 关键字来修饰变量
        ...
        alert(aa);
*****************************************
     如果不用var关键字来修饰的变量，并且这个所谓的变量也没有赋值，那么javascript会      报错。
     如果不用var关键字来修饰的变量，但是变量赋值了，那么javascript会把他当作一个        全局变量
     来处理，不会报错。但是我们不推介使用。
*****************************************
